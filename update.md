...
При нажатии **ЛКМ** по шаблону, будем произведён бросок, результат которого отразится в чате:
...
- **Продать предмет**: при нажатии **ЛКМ** на монеты, находящиеся напротив отдельного предмета, если Мастер Игры включил настройку "Мародёрство. Быстрая продажа" произойдёт следующее:
  ...
  - Если у игрока, нажавшего на предмет, есть связанный актёр, монеты, в количестве, равном стоимости предмет, буду добавлены в его инвентарь.
...
- **Забрать все предметы**: при нажатии **ЛКМ** на слово "Добыча", расположенную над списком предметов:
...
- **Монеты**: при нажатии **ЛКМ** на слово "Монеты", расположенную над списком предметов:
...
- Количество: определяет количество монет или предметов, если шанс сработает. Если для кубов d10, d20 и d100 выпадается максимальное количество, совершается дополнительный бросок количества.
- Таблицы: наборы предметов, которые могут быть добавлены в результате:
...
Для отображения шаблонов Генератора Предметов, необходимо указать тип предмета. Список типов предмета:
...
_Использование разделения всех предметов на типы обусловлено лишь удобством поиска для конкретных предметов._
...
При наведении на шаблон, будет отображено его описание. Оно также включает и список входящих в шаблон достоинств, недостатков (их соотношение отображается) и скриптов.
...
Когда предмет указан, при выборе хотя бы одного шаблона, появится кнопка **Создать**.
